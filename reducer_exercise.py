...
# The key is the category type and the value is the sales

# Example input data (Key=Category, Value=Sales)
# Input is ordered by the key

# We want to sum all values with the same key
# Example output data (Key=Category, Value=Sum of Sales)
...
for line in sys.stdin:
    ...
    if previous_key != None and previous_key != key:
        ...
        #Exercise 5: Categories with total number of purchases > 114
	...
	#Exercise 6: Caluclate the avg sales of each category = sales/counts
	#Avg starts with 0
	avg_of_sales = 0
    ...
    #for counting (Exercise 2) we are not using += (value) anymore. We just use += 1 to count the lines per Category
    count_of_values += 1
...
#Exercise 5: Show Categories with only total (count) number > 114 purchases
if count_of_values > 114:
	sys.stdout.write("{0}\t{1}\n".format(previous_key, avg_of_values))
